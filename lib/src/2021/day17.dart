...
extension on (Position a, Position b) {
  bool contains(Position position) => position.x >= this.$0.x && position.x <= this.$1.x && position.y >= this.$0.y && position.y <= this.$1.y;
}
    ...
    for (var x = 0; x < rangeX; x++) {
      for (var y = 0; y < rangeY; y++) {
        ...
        for (var i = 0; i < maxIt; i++) {
          ...
          if (position.y < min(zone.$0.y, zone.$1.y)) break;
          if (position.x > max(zone.$0.x, zone.$1.x)) break;
        }
      } 
    }
